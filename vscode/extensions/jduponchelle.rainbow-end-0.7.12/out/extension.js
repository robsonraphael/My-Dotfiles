...
function createDeepDecorations() {
    const colors = vscode.workspace.getConfiguration(configSection).get('colors');
    const mappedColors = colors === null || colors === void 0 ? void 0 : colors.map((color) => vscode.window.createTextEditorDecorationType({ color }));
...
function activate(context) {
    Object.keys(languages_1.languages).forEach(language => {
...
function buildRegex(language) {
    const languageConfiguration = languages_1.languages[language];
    let tokens = languageConfiguration["openTokens"];
...
function ignoreInDelimiters(token_pairs, text) {
...
function updateDecorations() {
    ...
    const languageConfiguration = languages_1.languages[activeEditor.document.languageId];
